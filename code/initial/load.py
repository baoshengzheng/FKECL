...
# 处理RotatE_128_64.pkl文件  得到
def fill1(name2id, loaded_id2name, emb, loaded_emb):
...
def fill2(name2id, loaded_id2name, emb, loaded_emb):
    for idx, name in loaded_id2name.items():
        real_idx = name2id.get(name, None)
        if real_idx is None:
            continue
...
def load(file_name, total_entity2id, total_relation2id, e_dim=128, r_dim=128) -> (torch.Tensor, torch.Tensor):
    ...
    loaded_relation_emb, loaded_id2relation = loaded_dict['relation'], loaded_dict['id2relation'] # 将所有数据中的关系转化成向量   以及id：关系名形式
    loaded_entity_emb, loaded_id2entity = loaded_dict['entity'], loaded_dict['id2entity']         # 将所有数据中的实体转化成向量   以及id：实体名形式
    relation_emb = nn.Embedding(len(total_relation2id), r_dim).weight.data # 做 7×64 维 的空向量组  用于装关系向量  每个关系用64维表示
    entity_emb = nn.Embedding(len(total_entity2id), e_dim).weight.data     # 做一个 115×128维的空向量组 用于装实体向量  每个节点用128维表示

    relation_emb = fill1(total_relation2id, loaded_id2relation, relation_emb, loaded_relation_emb)  # 把传入数据按照总库中对应的转换
    entity_emb = fill2(total_entity2id, loaded_id2entity, entity_emb, loaded_entity_emb)
    # import pdb; pdb.set_trace()
    # RotatE_128_64_emb.pkl
