...
class GCNNodeEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.gnn = GCN(in_feats=args['node_indim'],  # 输入128维 
                       hidden_feats=[args['hidden_feats']] * args['num_gnn_layers'], # 512 * 4层
        ...
        self.out_dim = gnn_out_feats  # 输出 512维 

        self.node_emb = nn.Embedding(343, args['node_indim'])  # 128 维
        self.edge_emb = nn.Embedding(21, args['edge_indim'])

    def forward(self, bg):
        node_feats = self.gnn(bg, self.node_emb(bg.ndata['h']))  # 刚传进来的128 维向量
        return node_feats
