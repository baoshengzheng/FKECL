...
class KMPNNGNN(nn.Module):
    def __init__(self, args, entity_emb, relation_emb):
        ...
        self.gnn_layer = KMPNN(
            in_feats=args['node_hidden_feats'],  # 64
            out_feats=args['node_hidden_feats'], # 64
            attn_fc=attn_fc,
            edge_func1=edge_network1,
            edge_func2=edge_network2,
            aggregator_type='sum'
        )
        self.gru = nn.GRU(args['node_hidden_feats'], args['node_hidden_feats'])  # 64 64
        self.out_dim = args['node_hidden_feats']    # 64
    ...
    def forward(self, g):
        ...
        node_feats = self.project_node_feats(node_feats) # (V, node_out_feats) (V,64)
        hidden_feats = node_feats.unsqueeze(0)           # (1, V, node_out_feats)
